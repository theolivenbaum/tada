...
if __name__ == "__main__":
    ...
    # 1. A directory with unconsolidated FSDP checkpoint (will be auto-consolidated)
    # 2. A consolidated .ckpt file
    #
    # To manually consolidate beforehand:
    # python -m lightning.pytorch.utilities.consolidate_checkpoint /path/to/checkpoint_folder
    # ckpt_path = "/mnt/weka/models/outputs/cont_tokens/llm_sync_tok/ft/3b_filtered_ce_long/epoch=9-step=100000.ckpt"
    # ckpt_path = "/mnt/weka/models/outputs/cont_tokens/llm_sync_tok/ft/3b_filtered_ce_long_convo/epoch=0-step=39000.ckpt"
    # ckpt_path = (
    #     "/mnt/weka/models/outputs/cont_tokens/llm_sync_tok/ft/3b_filtered_ce_long_convo_text/epoch=0-step=48000.ckpt"
    # )
    ckpt_path = (
        "/mnt/weka/models/outputs/cont_tokens/llm_sync_tok/ft/3b_filtered_ce_long_convo_ml_cfg/epoch=0-step=85000.ckpt"
    )
    ckpt_path = "/mnt/weka/models/outputs/cont_tokens/llm_sync_tok/ft/3b_filtered_ce_long_convo/consolidated.ckpt"

    ckpt_path = (
        "/mnt/weka/models/outputs/cont_tokens/llm_sync_tok/ft/3b_filtered_ce_long_convo_eng/epoch=0-step=73000.ckpt"
    )
    ...
    # Load checkpoint (will auto-consolidate if it's an unconsolidated FSDP checkpoint)
    ...
    if True:
        model.save_pretrained(os.path.join(get_weight_dir(), "uploads", "tada-3b"))

        api = HfApi()
        api.upload_folder(
            folder_path=os.path.join(get_weight_dir(), "uploads", "tada-3b"),
            repo_id="HumeAI/tada-3b",
            repo_type="model",
            delete_patterns="*",
        )
    else:
        output_path = os.path.join(os.path.dirname(ckpt_path), "exported")
